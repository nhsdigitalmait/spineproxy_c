...
void markSessionDone(session* s, int eof) {
	...
	/*
		If we got anything "residual", write it to the log file
	*/
	if (!s->transmission_details_received && s->buffer_read_ptr) {
		int w = write(s->log_file, s->buffer, (size_t)s->buffer_read_ptr);
		if (w < 0){
			perror("Error logging remaining data");
		}
	}
...
session* registerSession(char* client, int skt, forwarder_config *f) {
	...
	*/
	...
	s->buffer_size = f->session_buffer_size;
	s->sending = 0;
	s->processing_ack = 0;
	s->sending_ack = 0;
...
void getTransmissionLength(session* s) {
	...
	if (s->transmission_length >= s->forwarder->session_buffer_size) {
		perror("Offered transmission larger than forwarder buffer - resizing");
		s->buffer = (char*)realloc(s->buffer, (size_t)(s->transmission_length * sizeof(char)));
		memset(s->buffer + s->buffer_size, 0, (size_t)(s->transmission_length - s->forwarder->session_buffer_size));
		s->buffer_size = s->transmission_length * sizeof(char);
	}
}
